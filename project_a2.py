...
for batch_size in batch_sizes:
    ...
    w1.set_value(create_weights(36, 10))
    b1.set_value(create_bias(10))
    w2.set_value(create_weights(10, 6, logistic=False))
    ...
    #Plots
    plt.figure(1)
    plt.plot(np.arange(epochs), train_cost, label='b=%d'%batch_size)

    plt.figure(2)
    plt.plot(np.arange(epochs), test_accuracy, label='b=%d'%batch_size)
...
plt.title('Trainig cost for every batch size')
plt.savefig('figure/pa2_training_cost.png')

plt.figure(2)
plt.legend()
plt.xlabel('epochs')
plt.ylabel('accuracy')
plt.title('Test accuracy for every batch size')
plt.savefig('figure/pa2_test_accuracy.png')

plt.figure(3)
plt.plot(batch_sizes, time_for_update[batch_sizes])
plt.xlabel('batch size')
plt.ylabel('time for update in ms')
plt.title('time for a weight update')
plt.xticks(batch_sizes)
plt.savefig('figure/pa2_time_for_a_weight_update.png')
